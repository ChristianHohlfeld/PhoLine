/* Popup — enable toggle + last-request before/after estimate. No count.js. */
(function () {
  "use strict";
  ...
  chrome.storage.local.get(
    ...
    (s) => {
      ...
      function renderDetail() {
        if (!last) return;
        const before = last.fromText || "(kein Text — einmal neu senden)";
        const after = last.toText || last.toFull || "(kein Text — einmal neu senden)";
        if (view === "before") {
          tabBefore.classList.add("active");
          tabAfter.classList.remove("active");
          detailMeta.textContent =
            "Unbearbeitet · ≈ " +
            (typeof last.fromTokens === "number" ? last.fromTokens : "?") +
            " · " +
          ...
          detailMeta.textContent =
            "Danach · ≈ " +
            (typeof last.toTokens === "number" ? last.toTokens : "?") +
            " · " +
            savingsLine(last).replace(/^Ersparnis\s+/, "Ersparnis ");
      ...
      if (last && typeof last.fromTokens === "number") {
        ...
        const lines = [
          "Letzte Anfrage  ≈ " + last.fromTokens + " → " + last.toTokens + "  ▸",
          savingsLine(last) + "  · Schätzung",
        ];
